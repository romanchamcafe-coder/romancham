...
-- ---------- One-time fix for kitchen.romancham@gmail.com ----------
-- Switch off the empty workspace it owns, keep it as Kitchen in Romancham Cafe.
update public.memberships m set is_active = false, status = 'removed'
where m.user_id = (select id from auth.users where lower(email) = 'kitchen.romancham@gmail.com')
  and m.role::text = 'owner'
  and not exists (select 1 from public.memberships o where o.org_id = m.org_id and o.user_id <> m.user_id and o.is_active);

update public.memberships set role = 'kitchen', is_active = true, status = 'active'
where org_id = 'cf2dbc8f-9d4b-4c37-b408-81b0defd82da'
  and user_id = (select id from auth.users where lower(email) = 'kitchen.romancham@gmail.com');

-- Check: expect ONE active row -> Romancham Cafe | kitchen | true
select o.name, m.role::text as role, m.is_active
from public.memberships m
join auth.users u on u.id = m.user_id
join public.organizations o on o.id = m.org_id
where lower(u.email) = 'kitchen.romancham@gmail.com'
