...
export default async function NewPurchasePage() {
  ...
  const { vendors, ingredients, branches, units, categories } = await getPurchaseFormData(ctx!.orgId!);
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">New Purchase</h1>
      <PurchaseForm vendors={vendors} ingredients={ingredients} branches={branches} units={units} categories={categories} defaultBranchId={ctx!.branch?.id ?? ""} />
    </div>
  );
}
